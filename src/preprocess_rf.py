...
def fetch_county_dataset():
    ...
    state_fips = place_suffix.where(place_suffix.str.len() >= 2).str[:2]
    df["state_fips"] = state_fips

    if {"Count_Person_BachelorDegreeOrHigher", "Count_Person"}.issubset(df.columns):
        denom = df["Count_Person"].replace(0, np.nan)
        df["BachelorDegreeRate"] = df["Count_Person_BachelorDegreeOrHigher"] / denom

    if "Median_Income_Household" in df.columns:
        df["Log_Median_Income_Household"] = np.log1p(df["Median_Income_Household"].clip(lower=0))

    if "PopulationDensity" in df.columns:
        df["Log_PopulationDensity"] = np.log1p(df["PopulationDensity"].clip(lower=0))

    if {"UnemploymentRate_Person", "PovertyRate_Person"}.issubset(df.columns):
        df["EconomicDistressIndex"] = (
            df["UnemploymentRate_Person"] + df["PovertyRate_Person"]
        ) / 2.0

    return df


def save_data(df, filename="county_dataset_rf.csv"):
...
def split_data(df, target_column):
    ...
    if y.nunique(dropna=True) >= 10:
        try:
            ...
            if getattr(stratify_labels, "nunique", lambda: 0)() < 2:
                stratify_labels = None
        except Exception:
            stratify_labels = None
